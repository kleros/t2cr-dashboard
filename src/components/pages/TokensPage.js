...
class TokensPage extends React.Component {
    ...
    render() {
        const { accepted, rejected, crowdfunding, pending, challenged, appealed, total } = this.props.tokensCountByStatus;
        ...
        return (
            <div className="App__Page TokensPage">
                ...
                <InfoPanelWrapper>
                    <InfoPanel type="token" img={tokenAcceptedImg} imgText="Accepted" label="Accepted" value={accepted}/>
                    <InfoPanel type="token" img={tokenRejectedImg} imgText="Rejected" label="Rejected" value={rejected}/>
                    <InfoPanel type="token" img={tokenCrowdfundingImg} imgText="Crowdfunding" label="Crowdfunding" value={crowdfunding}/>
                    <InfoPanel type="token" img={tokenPendingImg} imgText="Pending" label="Pending" value={pending}/>
                    <InfoPanel type="token" img={tokenChallengedImg} imgText="Challenged" label="Challenged" value={challenged}/>
                    <InfoPanel type="token" img={tokenAppealedImg} imgText="Appealed" label="Appealed" value={appealed}/>
                </InfoPanelWrapper>
            </div>
        );
    }
...
const mapStateToProps = state => {
    ...
    if (depositData !== null && priceEth !== null) {
        tokensTotalUsd = parseFloat(Math.round(depositData.tokensTotalEth * priceEth * 100) / 100).toFixed(2)
        tokensTotalEth = parseFloat(Math.round(depositData.tokensTotalEth * 100) / 100).toFixed(2);
     }
